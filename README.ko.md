...
- **종단간 암호화 iCloud 동기화(베타), 계정도 서버도 필요 없이.** iCloud Drive 폴더를 통해 텍스트·이미지 히스토리를 — 보드로 정리한 구조까지 — 내 Mac끼리 동기화합니다. 켜는 순간 이미 쌓인 것도 한 번에 올라갑니다. 폴더에 쓰이는 모든 것은 각 Mac에서 한 번 정하는 동기화 암호에서 파생한 키로 AES-GCM 암호화돼서, 폴더를 열어도 나 말고는 못 읽습니다. 시크릿 보드는 설계상 로컬에만 남고, 파일은 동기화하지 않습니다. 기본은 꺼짐.
